...
def GdbCommand(command):
    ...
    ser.write(('{}'.format(checksum(command))).encode())
    if command == 'c':
        return ''
    out = ''
    char = ''
    ...
    while newrle[0] == "|" or newrle[0] == "+" or newrle[0] == "$":
        newrle = newrle[1:]
    return bytes.fromhex(newrle)

def OnReadReg():
    regs =  DisplayRegistersMIPS(GdbCommand('g'))
    print('All registers:')
    for k, reg_name in reg_map.items():
...
def OnSearchMem(addr, pattern):
    ...
    while True:
        ...
        if pattern in buf[-100:-1]:
            ...
            return
        cur_addr = pack(">I", unpack(">I",cur_addr.decode('hex'))[0] + 0xc7).hex()
...
def OnNext():
    ...
    pc_in_hex = pack('>I', pc_after_branch).hex()
...
def OnDumpMemory(start, stop):
    ...
    if not isValidDword(start) or not isValidDword(stop):
        ...
        return
    cur_addr = start
    while unpack(">I",bytes.fromhex(cur_addr))[0] < unpack(">I", bytes.fromhex(stop))[0]:
        res = GdbCommand('m{},00c7'.format(cur_addr))
        logger.info('Dumping at {} len {}'.format(cur_addr, len(res)))
        cur_addr = pack(">I", unpack(">I",bytes.fromhex(cur_addr))[0] + 0xc7).hex()
        buf += res
    return buf
